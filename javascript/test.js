...
function changeMode(){
    switch(intMode){
            case 1:
                document.getElementById('modeId').innerHTML = 'Mode : Manual send';
                clearInterval(myVar);
                break;
            case 2:
                document.getElementById('modeId').innerHTML = 'Mode : Automatic send';
                clearInterval(myVar);
                break;
            case 3:
                document.getElementById('modeId').innerHTML = 'Mode : Mobile auto mode';
                myVar = setInterval(myTimer, 2000);
                document.getElementsByClassName('mode').value = true;
                break;
        }
...
$(function() {
  ...
  $(document)
    ...
    // Bind translate function in interval  
    .on('click', 'button.backMode', function() {
        intMode--;
        if(intMode < 1) { intMode = 3;}
        changeMode();
    })
    ...
    // On change button 
    .on('click', 'button.mode', function() {
        // Manual mode = 1 first
        intMode++;
        if(intMode > 3) { intMode = 1;}
        changeMode();        
        ...
        //     document.getElementById('modeId').innerHTML = 'Mode : Manual send';
        //     clearInterval(myVar);
        //     document.getElementsByClassName('mode').value = false;
